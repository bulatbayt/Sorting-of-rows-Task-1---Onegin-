...
int HasLetter (const char* string);
...
int main(void)
{
    struct File_info Onegin = { }; 
 
    FILE* clearing_file_contents = fopen ("Onegin_write.txt", "w");

    fclose (clearing_file_contents);
    ...
    buble_sort (index, Onegin.max_rows, CompareFuncDown); 
...
{
    hand_assert (name_file != NULL, "name_file != NULL",  __LINE__, __FILE__);
    hand_assert (index != NULL, "index != NULL", __LINE__, __FILE__);
    ...
    // TODO: use arrows
    *buffer = calloc (Onegin->size_file, sizeof(char)); 

    fread (*buffer, sizeof(char), Onegin->size_file, onegin_file); 
...
{
    hand_assert (index != NULL, "index != NULL",  __LINE__, __FILE__);
    hand_assert (ReasonPrint != NULL, "ReasonPrint != NULL",  __LINE__, __FILE__);

    // TODO: google {w,r,a}+ -- achieved
    ...
    {
        if (HasLetter (index[i]) == 1)
        {
        fprintf(file_write, "%s", index[i]);
        }
    }
    fprintf (file_write, "\n-----------------------------------------------------------------\n\n");
...
size_t my_strlen (const char* string)
{
    hand_assert (string != NULL, "string != NULL",  __LINE__, __FILE__);
...
int my_strcmp_alpha_up (const char* s1, const char* s2) 
{
    hand_assert (s1 != NULL, "s1 != NULL",  __LINE__, __FILE__);
    hand_assert (s2 != NULL, "s2 != NULL",  __LINE__, __FILE__);

    int strlen_s1 = (int) my_strlen(s1);
    int strlen_s2 = (int) my_strlen(s2);
    ...
    while (1000-7 > 0)
    {
        while (s1[i1] != '\0' && !isalpha(s1[i1])) 
        {
            i1++;
        }

        while (s2[i2] != '\0' && !isalpha(s2[i2]))
        {
            i2++;
        }
...
{
    hand_assert (StringLeft != NULL, "StringLeft != NULL",  __LINE__, __FILE__);
    hand_assert (StringRight != NULL, "StringRight != NULL",  __LINE__, __FILE__);
...
int   my_strcmp_alpha_down (const char* s1, const char* s2)
{
    ...
    while (1000-7 >0)
    {
        ...
        // TODO: correct handling achieved
        /*if (i1 < 0 || i2 < 0)
        {
            return 0;
        }*/
...
void change_index(char** p1, char** p2) 
{
    hand_assert (p1 != NULL, "p1 != NULL",  __LINE__, __FILE__);
    hand_assert (p2 != NULL, "p2 != NULL",  __LINE__, __FILE__);
...
int CompareFuncOriginal (const void* PointerLeft, const void* PointerRight)
{
    hand_assert (PointerLeft != NULL, "PointerLeft != NULL", __LINE__, __FILE__);
    hand_assert (PointerRight != NULL, "PointerLeft != NULL", __LINE__, __FILE__);

    const char* s1 = *(const char**) PointerLeft; 
    const char* s2 = *(const char**) PointerRight;

    // FIXME: use if (or ssize_t)
...
int hand_assert (int result_expression, char* string, int line_of_string, char* file_error)
{
    ...
    {
        printf ( "ERROR %s in %d line, %s", string, line_of_string, file_error);
        return 1;
    }
